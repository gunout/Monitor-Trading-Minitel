...
ASSETS = {
    ...
    'FP.PA': {'name': 'TotalEnergies', 'exchange': 'Euronext', 'category': 'Paris Stocks', 'icon': '⛽', 'color': '#0033a0'},
    ...
    'VE': {'name': 'Veolia Environnement', 'exchange': 'NYSE', 'category': 'Eau', 'icon': '💧', 'color': '#00a3a3'},
}
    ...
    if indicators['last_macd'] is not None and indicators['last_macd_signal'] is not None and len(indicators['macd']) > 1:
        ...
        if prev_macd is not None and prev_signal is not None:
            if prev_macd < prev_signal and indicators['last_macd'] > indicators['last_macd_signal']:
                ...
                score -= 15
        elif indicators['last_macd'] > indicators['last_macd_signal']:
            signals.append({'type': 'buy', 'indicator': 'MACD', 'value': f"{indicators['last_macd']:.3f}", 'message': 'MACD > Signal'})
            score += 10
        elif indicators['last_macd'] < indicators['last_macd_signal']:
            signals.append({'type': 'sell', 'indicator': 'MACD', 'value': f"{indicators['last_macd']:.3f}", 'message': 'MACD < Signal'})
            score -= 10
...
def get_insights(symbol):
    try:
        ...
        set_cached(f"insights_{symbol}", indicators)
...
def handle_request_insights(data):
    ...
    try:
        with app.app_context():
            ...
            if isinstance(insights, tuple):
                insights = insights[0]
            if insights and not isinstance(insights, dict):
                insights = jsonify(insights).get_json()
            emit('insights_update', insights)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("📊 TRADING MONITOR - Indicateurs Techniques + IA")
    print("=" * 70)
    print("🌐 http://localhost:5000")
    print("=" * 70)
    print("📈 Indicateurs disponibles (boutons sur le graphique):")
    print("   🔵 SMA 20/50/200")
    print("   🟣 EMA 12/26")
    print("   🟡 RSI (14)")
    print("   🔴 MACD")
    ...
    print("=" * 70)
    print("🤖 Insights IA:")
    print("   📊 Score de -100 à +100")
    print("   📊 Recommandation ACHAT/VENTE/NEUTRE")
    print("   📊 Signaux détectés")
    print("   📊 Prédictions 5 jours")
